...
for t in config:
    if t in ['version', 'EARTHDATA_u', 'EARTHDATA_p']: continue
...
if ('EARTHDATA_u' in config) & ('EARTHDATA_p' in config):
    if len(config['EARTHDATA_u']) > 0:
        os.environ['EARTHDATA_u'] = config['EARTHDATA_u']
    if len(config['EARTHDATA_p']) > 0:
        os.environ['EARTHDATA_p'] = config['EARTHDATA_p']
